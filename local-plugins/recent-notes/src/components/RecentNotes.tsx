...
export default ((userOpts?: Partial<RecentNotesOptions>) => {
  const RecentNotes: QuartzComponent = ({
    ...
    // gpunkt.org only shows this on the index page — baked in here rather than via a
    // ConditionalRender/quartz.ts override, since loadQuartzLayout()'s per-pageType override
    // merge is shallow (whole-array replace, not append), which would be fragile against future
    // YAML afterBody changes.
    ...
    const pages = filterListedPages(allFiles as RecentNotesPluginData[])
      ...
      .filter((p) => p.slug !== "index") // v4's filter option can't be expressed in YAML
      .filter(opts.filter)
      .sort(opts.sort);
